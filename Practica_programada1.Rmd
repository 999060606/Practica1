...
Ajuste los nombres de las columnas para que estén en español. (2pts)
...
```{r}
duplicados <- get_dupes(datos) ##que registros estan duplicados
```
...
```{r}
unique(datos_limpios$Tipo) #funcion para verificar si se arreglaron los cambios de las variables
```
